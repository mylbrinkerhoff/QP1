...
\ea Blocking by verb
	...
	\ex[*]{ 
	\gll Peter har \textit{ham\sub{o}} ikke [\sub{VP} \textbf{sått} \textbf{t\sub{o}}] \\
...
Assuming that Match Theory \citep{selkirkClauseIntonationalPhrase2009,selkirkSyntaxPhonologyInterface2011} is a correct way of understanding the relationship between syntax and prosody, there are several predictions that this theory makes when mapping the syntax to the prosody. Under Match Theory a XP corresponds to a phonological phrase unless that XP is both minimal and maximal, in which case, it forms a prosodic word \citep{bennettLightestRightApparently2016} or the XP is a CP then it forms a intonational phrase.
\begin{table}[!h]
...
LMC allowing for the deletion of the offending material will allow for the correct size of the material to be pronounced in the \emph{Mittelfeld}. Once the LMC has done its work the standard notion of PF spell-out will preserve the portion of the shifted NI that remains in the high copy while deleting that portion in the lower copy. This works if the LMC operates left-to-right after linearization has taken place but prior to full pronunciation. The difference that we observe with Norwegian's lack of NegShift comes down to the fact that we instead have covert movement in LF. This is due to the [$u$\textsc{Neg}] feature being weak and will get valued during LF, similar to what \citet{zeijlstraSyntacticallyComplexStatus2011} proposes for NIs in German, Dutch, and English.
...
One way to think about this comes down to the fact that in Swedish determiners and NPs form a single maximal prosodic word as evidenced by the tonal melodies on the resulting prosodic word. In the case of Swedish, when the NI and the NP coalesce we have tonal accent 2 which corresponds with a word compounds (see discussion in \cite{myrbergProsodicWordSwedish2013,myrbergProsodicHierarchySwedish2015} for more information about word compounds in Swedish). The fact that in Swedish we get a tonal accent 2 tells us that instead of being composed of a clitic and its host we instead have a prosodic structure in it seems that the resulting prosodic structure would contain two minimal prosodic words. Evidence for this comes from the fact that both the NI and the NP bear stress, which is a characteristic restricted to the minimal prosodic word. 
\ea
\begin{forest}
	[$\omega_{max}$
...
\end{forest} 
\z 
It seems then that Swedish has the preference of only having minimal prosodic words instead of maximal prosodic words in the \emph{Mittelfeld}. The reason that NI pronouns do not present a problem in Swedish is because they are both maximal and \emph{minimal} prosodic words. 
